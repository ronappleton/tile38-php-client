...
`tile38.call()` and `tile38.pcall()`, and are useful for multi-step operations that
must run atomically without round trips.
...
The second argument is the number of **keys** that follow. Keys land in the `KEYS`
table, and extra arguments in `ARGV`:
...
The `-sha` variants run a script cached on the server by its SHA1 digest, which skips
...
Global variables cannot be created (use `local`). Script caching is in-memory and is
lost when the server restarts.

## Tutorial

See [Realtime Dispatch](/tutorials/driver-dispatch/) for scripting in a real workflow.

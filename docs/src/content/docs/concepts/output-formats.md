...
`GET` returns the full object by default, but you can ask for a specific
projection:
...
By default objects are returned without their fields. Use `withfields()` to
include them:
...
`withfields()` is also available on `GET`, and `NOFIELDS` on search commands
does the reverse (drops fields from results).
